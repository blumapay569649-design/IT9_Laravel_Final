...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('inventory.update', ['item' => $item]) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control @error('capital_price') is-invalid @enderror" id="capital_price" name="capital_price" value="{{ old('capital_price', $item->capital_price ?? 0) }}" placeholder="0.00">
                        @error('capital_price') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-3">
                        <label for="sell_price" class="form-label">Sell Price</label>
                        <input type="number" step="0.01" class="form-control @error('sell_price') is-invalid @enderror" id="sell_price" name="sell_price" value="{{ old('sell_price', $item->sell_price ?? 0) }}" placeholder="0.00">
                        @error('sell_price') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-3">
                        <label for="category" class="form-label">Category</label>
                        <input type="text" class="form-control @error('category') is-invalid @enderror" id="category" name="category" value="{{ old('category', $item->category) }}" placeholder="e.g., Electronics, Food, etc.">
                        @error('category') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-3">
                        <label for="quantity_kilo" class="form-label">Quantity</label>
                        <input type="number" step="0.01" class="form-control @error('quantity_kilo') is-invalid @enderror" id="quantity_kilo" name="quantity_kilo" value="{{ old('quantity_kilo', $item->quantity_kilo) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        @if($item->image_path)
                            <small class="text-muted">Current: {{ $item->image_path }}</small>
                        @endif
                    </div>
